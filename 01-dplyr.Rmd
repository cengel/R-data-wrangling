...
```{r, results = 'show', purl = FALSE, echo=FALSE}
select(stops, starts_with("driver")) %>% head()
```
...
```{r, results='show', purl = FALSE, echo=FALSE}
filter(stops, county_name == "Yazoo") %>% head()
```

Here are some other ways to select rows:

- select certain rows by row number: `slice(stops, 1:3) # rows 1-3`
...
To sort rows by variables use the `arrange` function: `arrange(stops, county_name, stop_date)`
...
```{r, eval=FALSE}
select(filter(stops, driver_age > 85), violation_raw, driver_gender)
```
...
In the above, we use the pipe to send the `stops` dataset first through
...
> <h3>Challenge</h3>
>
>  Using pipes, subset the `stops` data to include stops in Tunica only and retain the columns `stop_date`, `driver_age`, and `violation_raw`. Bonus: sort the table by driver age.
...
## Tunica only and retain the columns `stop_date`, `driver_age`,
