...
library(lubridate)
MS_county_stops <- read.csv('data_output/MS_county_stops.csv')
...
(If you need to, you can also download the data from here: http://bit.ly/MS_county_stops. Create a folder `data_output` in your working directory then download with  `download.file('http://bit.ly/MS_county_stops', 'data_output/MS_county_stops.csv`)
...
MS_plot <- ggplot(data = MS_county_stops, aes(x = female, y = male))

# Draw the plot
MS_plot + geom_point()
```

Also, conveniently, `ggplot` works with pipes. The code below does the same thing as above:
...
We pipe the content of the table into `ggplot()`, so we can omit the first (`data =` argument). 
...
```{r adding-transparency, results='show', purl=FALSE, out.width='70%'}
  ggplot(MS_county_stops, aes(x = female, y = male)) + 
...
```{r color-bar-stretch-flip, results='show', purl=FALSE, out.width='70%'}
ggplot(trafficstops, aes(violation)) + 
  geom_bar(aes(fill = driver_gender), position = "fill") +
  coord_flip()
```
...
The other type of barchart, `geom_col`, is used if you want the heights of the bars to represent the actual data values. In this example, we first generate a summary statistics, and then pipe it into ggplot to visualize the values we have calculated.


```{r demograph-colplot, results='show', purl=FALSE}
trafficstops %>% 
    group_by(violation) %>% 
...
```{r demograph-colplot-reorder, results='show', purl=FALSE}
trafficstops %>% 
  group_by(violation) %>% 
  summarize(mean_age = mean(driver_age, na.rm = TRUE)) %>%
...
## Challenge: Make a barplot that shows the proportion of stops per race for within each gender category. How could you get rid of the NAs?
...
Yazoo_stops <- trafficstops %>% 
  filter(county_name == "Yazoo County",
         !is.na(driver_age))
```
...
To make things a little easer we first convert the `stop_date` column we plan to use to Date format and use the `wday` function to extract and add a new column to our `trafficstops` data called `wk_day` with the weekday for each of these dates. For better understanding we will label the weekdays.

```{r add-weekday-col, purl=FALSE}
library(lubridate)
...
```{r, count-weekly-violations, results='show', purl=FALSE}
trafficstops %>%
  count(wk_day, violation) 
```

We visualize thise data as a line plot (with -- you guessed it -- `geom_line()`) mapping the days to the x axis and counts to the y axis. So we pipe the output from above into ggplot like this:
...
grey_theme <- theme(axis.text.x = element_text(colour="grey40", 
                                               size=12, angle=90, 
                                               hjust=.5, vjust=.5),
                   axis.text.y = element_text(colour="grey40", size=12),
                   text=element_text(size=16))
