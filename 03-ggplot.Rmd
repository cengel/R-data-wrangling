...
- Any parameters you set in the `geom_*()` functio nare treated independently of (and override) the settings defined globally in the `ggplot()` function. 
...
> <h3>Challenge</h3>
>
> Make a plot that shows for each race the proportion of stops for male and female drivers. How could you get rid of the NAs?

```{r, echo=FALSE, eval=FALSE, purl=TRUE}
## Challenge: make a plot that shows the percentage of stops per race for each gender. How could you get rid of the NAs?
...
##  2. Create boxplot for `driver_race` overlaid on a jitter layer.
```
...
Let's calculate number of counts per weekday for each violation. For better understanding we will label the weekdays. First we need to group the data and count records within each group:
```{r, purl=FALSE}
trafficstops %>% 
...
> Hint: make sure you just use records where driver_age > 16.
...
## Use what you just learned to create a plot that depicts how the average age
## of each driver for the two recorded ethnicities changes through the week.
## Hint: make sure you filter out the records with driver_age > 16.

```

<!-- Answer
```{r average-age-time-series, purl=FALSE}
trafficstops %>%
   filter(driver_age > 16) %>% 
   mutate(wk_day = wday(stop_date, label = TRUE)) %>%  
   group_by(wk_day, driver_race) %>%
